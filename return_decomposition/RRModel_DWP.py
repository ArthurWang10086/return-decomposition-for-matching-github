...
class RRModel(object):
    def __init__(self, state_shape, n_actions, max_timestep, n_batch=1,
        ...
        return_internal = tf.matmul(tf.reduce_sum(rr_returns['predictions'],[2]),tf.matrix_band_part(tf.ones([n_timesteps+1,n_timesteps+1]), 0, -1))
        AUCW = tf.reduce_mean(tf.abs(tf.tile(tf.reshape(true_return,[-1,1]),[1,n_timesteps+1]) - tf.matmul(tf.reduce_sum(rr_returns['predictions'],[2]),tf.matrix_band_part(tf.ones([n_timesteps+1,n_timesteps+1]), 0, -1))))
        reward_prediction_error = AUCW
        # true_return = tf.matmul(tf.reduce_sum(rr_returns['predictions'],[2]),tf.matrix_band_part(tf.ones([n_timesteps+1,n_timesteps+1]), 0, -1))
        ...
        def train(states, actions, rewards, ):
            ...
            loss, true_ret, pred_ret, _,AUCW_, summary,true_return_,return_internal_ = sess.run(
                [total_loss, true_return, return_prediction, rr_update,AUCW, summaries,true_return,return_internal], feed_dict=feed_dict)

            return {'loss': loss, 'true_return': true_ret, 'predict_return': pred_ret,
                    'summaries': summary,'AUCW':AUCW_,'true_return':true_return_,'return_internal':return_internal_}
