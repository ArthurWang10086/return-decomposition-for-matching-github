...
EncodeMap1 = {'Steal': 1, 'ReserveAction': 2, 'ShootCancel': 3, 'ShootResult': 4, 'Pass': 5, 'Rebound': 6, 'Shoot': 7, 'Block': 8, 'CheckAninTime': 9, 'Conversion': 10}


EncodeMap2 = {'Pass#None': 1, 'Block#success': 2, 'Block#fail': 3, 'ShootResult#2': 4, 'CheckAninTime#None': 5, 'Steal#fail': 6, 'ShootCancel#None': 7, 'Rebound#success': 8, 'Rebound#fail': 9, 'ReserveAction#None': 10, 'ShootResult#3': 11, 'Steal#success': 12, 'ShootResult#0': 13, 'Conversion#None': 14, 'Shoot#None': 15}


EncodeMap3 = {'ShootResult#0#near': 1, 'Pass#None#pass_long_lose': 2, 'Block#fail#None': 3, 'ShootResult#0#middle': 4, 'Rebound#success#normal': 5, 'Rebound#fail#normal': 6, 'Conversion#None#steal_catch': 7, 'ShootResult#2#three_point': 8, 'Rebound#fail#block_heighball': 9, 'Rebound#fail#block_highball': 10, 'Conversion#None#rebound': 11, 'Conversion#None#pickup': 12, 'Rebound#success#block_highball': 13, 'ShootResult#0#unknown': 14, 'Pass#None#intercept': 15, 'ShootResult#0#layupndunk': 16, 'Pass#None#intercept_catch': 17, 'Conversion#None#block_catch': 18, 'Pass#None#pass_short': 19, 'Steal#fail#None': 20, 'ShootResult#2#middle': 21, 'Conversion#None#BuzzerBeater': 22, 'Block#success#None': 23, 'ShootResult#2#layupndunk': 24, 'Steal#success#None': 25, 'ShootResult#2#near': 26, 'Conversion#None#goalin': 27, 'CheckAninTime#None#None': 28, 'Pass#None#pass_long_run': 29, 'ShootResult#3#three_point': 30, 'ReserveAction#None#None': 31, 'Shoot#None#None': 32, 'Pass#None#pass_short_nicepass': 33, 'Conversion#None#clock_violation': 34, 'ShootResult#0#three_point': 35, 'ShootCancel#None#None': 36}



# def encodefunc(x):
#     encodefunc.size = len(EncodeMap1)
#     return EncodeMap1['#'.join(x.split('#')[:4])] if '\n' not in x else EncodeMap1['#'.join(x[:-1].split('#')[:4])]
#
# def encodefunc2(x):
...
def encodefunc3(x):
    ...
    return EncodeMap3['#'.join(x.split('#')[:3])]-1
...
EncodeMapList = [encodefunc3]
# EncodeMapLen = len(EncodeMap1)+len(EncodeMap2)+len(EncodeMap3)
            ...
            for data in datas:
                for x in data.split(';'):
                    for action in x.split('@')[2].split(','):
                        if len(action.split(':')[-1])>0 and action.split(':')[-2] not in ['GameEnd','Skill','RecommendFriend','MentorsLog','MatchEnd','GameStart','GameObInfo']:
                            # actions.add(action.split(':')[-1])
                            actions.append('#'.join(action.split(':')[-1].split('#')[:3]))
    L=list(set(actions))
    print(actions)
    return dict(zip(L,range(1,1+len(actions))))


if __name__ == '__main__':
    result = actionEncode(['2019-03-02'])
    print(result)
